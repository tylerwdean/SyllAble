    ...
    <>
            ...
            {row.map((cell, cellIndex) => {
              return (
                <input
                  ...
                  value={cell}
                  onChange={(e) =>
                    updateCell(e.target.value, rowIndex, cellIndex)
                  }
                />
              );
            })}
            <button
              className="btn btn-secondary col-1 btn-sm"
              onClick={(e) => {
                e.preventDefault();
                removeRow(rowIndex);
              }}
      ...
      <div className="row mx-0 mt-2">
        ...
        {rows[0].map((_, index) => {
          return (
            <div
              className="col-sm mx-0 px-0"
              key={props.id + "-removeCol-" + index}
            >
              <button
                className="btn btn-secondary col-1-sm mx-0 px-3"
                disabled={oneCol}
                onClick={(e) => {
                  e.preventDefault();
                  removeCol(index);
      ...
      <button
        className="btn btn-danger col-4 offset-3 mt-3"
        onClick={props.deleteParagraph}
      >
        Delete Paragraph
      </button>
    </>
